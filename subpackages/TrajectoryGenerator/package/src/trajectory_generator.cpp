...
bool LemniscateGenerator::evaluateTrajectory(float time ,  std::array<std::array<float,3>,4>& refs){
    ...
    refs[0][1] = -a*sin(t)*((pow(sin(t),2) + 2*(pow(cos(t),2) + 1))/pow((pow(sin(t),2) + 1),2));
    refs[1][1] = -a*(pow(sin(t),4)* + pow(sin(t),2) + (pow(sin(t),2) - 1) * pow(cos(t),2))/pow(pow(sin(t),2) + 1,2);
    refs[2][1] = 0.0f;
    refs[3][1] = 0.0f;
    
    
    refs[0][2] = a*cos(t)*(5*pow(sin(t),4)+ 7*pow(sin(t),2) + (6*pow(sin(t),2) - 2)*pow(cos(t),2) - 2)/pow(pow(sin(t),2) + 1,3);
    refs[1][2] = (4*a*sin(t)*cos(t)*(pow(sin(t),6) + (pow(sin(t),2) - 1) *pow(cos(t),2)))/pow((pow(sin(t),2) + 1),3) - (a*(2*sin(t)*pow(cos(t),3) + 6*pow(sin(t),5)*cos(t) - 2*(pow(sin(t),2) - 1)*sin(t)*cos(t)))/pow(pow(sin(t),2) + 1,2);
    refs[2][2] = 0.0f;
    refs[3][2] = 0.0f;
